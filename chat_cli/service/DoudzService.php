...
class DoudzService
{
    ...
    //从指定的PK中找到比当前PK大的牌,在所有大的牌里面返回最小的牌面
    public function rtnbigpk($nowpk,$pkarr)
    {
        //$nowpk = ['c1', 'd13', 's12', 'h11', 'h10'];
        $nowpkTp = $this->rtnpktp($nowpk);
        $nowpklen = count($nowpk);
        ...
        $rtnArr = [];
        if(in_array($nowpklen,[1,2,3,4,5])){
            foreach($pkarr as $k=>$v){
                //针对一张，一对，三张，三带可以先不用自由组合，有单独的一张，一对，三张，三带最好
                $tmppkval = $this->pokerVal[$v];
                $newpkArr[$tmppkval][] = $v;
            }
            ...
            foreach($newpkArr as $k=>$v){
                $tmppklen = count($v);
                if($tmppklen == 1){
                    if(empty($verSmaOnepk)){
                        $verSmaOnepk = $v;
                    }else{
                        $tmpRs = $this->pokerarrpk($verSmaOnepk, $v);
                        if($tmpRs == 1){
                            $verSmaOnepk = $v;
                        }
                    }
                ...
                if($tmppklen == 2){
                    if(empty($verSmaTwopk)){
                        $verSmaTwopk = $v;
                    }else{
                        $tmpRs = $this->pokerarrpk($verSmaTwopk, $v);
                        if($tmpRs == 1){
                            $verSmaTwopk = $v;
                        }
                    }
                ...
                $newPkNumArr[$tmppklen][] = $v;
            }

            if($nowpklen == 1 && isset($newPkNumArr[1])){
                foreach($newPkNumArr[1] as $k=>$v){
                    $tmpRs = $this->pokerarrpk($nowpk, $v);
                    if ($tmpRs == 2) {
                        //$resArr[] = $v['pklist'];
                        if(empty($rtnArr)){
                            $rtnArr = $v;
                        }else{
                            $tmpRs = $this->pokerarrpk($rtnArr, $v);
                            if($tmpRs == 1){
                                $rtnArr = $v;
                            }
                        }
            ...
            if($nowpklen == 2 && isset($newPkNumArr[2])){
                foreach($newPkNumArr[2] as $k=>$v){
                    $tmpRs = $this->pokerarrpk($nowpk, $v);
                    if ($tmpRs == 2) {
                        //$resArr[] = $v['pklist'];
                        if(empty($rtnArr)){
            ...
            if($nowpklen == 3 && isset($newPkNumArr[3])){
                foreach($newPkNumArr[3] as $k=>$v){
                    $tmpRs = $this->pokerarrpk($nowpk, $v);
                    if ($tmpRs == 2) {
                        //$resArr[] = $v['pklist'];
                        if(empty($rtnArr)){
            ...
            if($nowpklen == 4 && isset($newPkNumArr[3]) && $nowpkTp['type'] == 3){
                if(empty($verSmaOnepk) && empty($verSmaTwopk)){

                }else{
                    $tmpOnePk = empty($verSmaOnepk)?$verSmaTwopk[0]:$verSmaOnepk[0];
                    foreach($newPkNumArr[3] as $k=>$v){
                        $tmpVpk = $v;
                        $tmpVpk[] = $tmpOnePk;
                        $tmpRs = $this->pokerarrpk($nowpk, $tmpVpk);
                        if ($tmpRs == 2) {
                            if(empty($rtnArr)){
                                $rtnArr = $tmpVpk;
                            }else{
                                $tmpRs = $this->pokerarrpk($rtnArr, $tmpVpk);
                                if($tmpRs == 1){
                                    $rtnArr = $tmpVpk;
                                }
                            }
            ...
            if($nowpklen == 5 && isset($newPkNumArr[3]) && $nowpkTp['type'] == 3){
                if(empty($verSmaTwopk)){

                }else{
                    foreach($newPkNumArr[3] as $k=>$v){
                        $tmpVpk = $v;
                        $tmpVpk = array_merge($tmpVpk,$verSmaTwopk);
                        $tmpRs = $this->pokerarrpk($nowpk, $tmpVpk);
                        if ($tmpRs == 2) {
                            if(empty($rtnArr)){
                                $rtnArr = $tmpVpk;
        ...
        if(empty($rtnArr)){
            $allPkLs = $this->getpktypebypkarr($pkarr, $nowpklen, $nowpkTp['type']);
            if (!empty($allPkLs)) {
                foreach ($allPkLs as $k => $v) {
                    $tmpRs = $this->pokerarrpk($nowpk, $v['pklist']);
                    if ($tmpRs == 2) {
                        //$resArr[] = $v['pklist'];
                        if(empty($rtnArr)){
                            $rtnArr = $v['pklist'];
                        }else{
                            $tmpRs = $this->pokerarrpk($rtnArr, $v['pklist']);
                            if($tmpRs == 1){
                                $rtnArr = $v['pklist'];
                            }
                        }
        ...
        if (empty($rtnArr) && $nowpkTp['type'] != 4) {
            $allPkLs = $this->getpktypebypkarr($pkarr, 4, 4);
            if (!empty($allPkLs)) {
        ...
        if (empty($rtnArr)) {
            $allPkLs = $this->getpktypebypkarr($pkarr, 2, 8);
            if (!empty($allPkLs)) {
                $rtnArr = $allPkLs[0]['pklist'];
            }
        }
...
//        $arr = ['h2','d2','s2','c9','d9'];$arr = ['h11','d12','s13','c9','d10'];
    ...
    public function pokerarrpk($pk1Arr,$pk2Arr){
        $pk1Arr = $this->rtnpktp($pk1Arr);
        $pk2Arr = $this->rtnpktp($pk2Arr);

        return $this->pokerpk($pk1Arr,$pk2Arr);
    }
    ...
    public function pokerpk($pk1Arr,$pk2Arr){
        ...
        $typeArr    = [1,2,4];//单张，对子，炸弹，四带二这几种只要对比最大牌面就好了
        if($pk1Arr['type'] == $pk2Arr['type']){
            ...
            //四带二对比时
            if($pk1Arr['type'] == 9 && $pk1Arr['type2'] == $pk2Arr['type2']){
                if($pk2Arr['max'] > $pk1Arr['max']){
                    return 2;
                }
            }
...
//        $arr = ['h2','d2','s2','c9','d9'];$arr = ['h11','d12','s13','c9','d10'];
        ...
        if($arrLen == 1){
            ...
            if($rtnArr['type'] == 0){
                ...
                if(count($pkValArr) == 3){
                    ...
                    if(isset($newPkArr[4]) && isset($newPkArr[1]) && $newPkArr[1] == 2){//四带二 这里有问题，有时间的时候处理下
                        $rtnArr['type']     = 9;
                        $rtnArr['type2']    = 2;
                        $rtnArr['pklist']   = $arr;
                        $rtnArr['max']      = $newPkArr[4];
        ...
        }else{//大于八张的，只能是顺子，连对，或者飞机 四带两对的逻辑少写了，有时间了补一下
            //顺子，飞机，连对
            if($rtnArr['type'] == 0){
                //判断是否是顺子
                $rtnArr = $this->is_cis($arr,$rtnArr);
            ...
            if($rtnArr['type'] == 0){
                ...
                if(count($pkValArr) == 3){
                    ...
                    if(isset($newPkArr[4]) && isset($newPkArr[2]) && $newPkArr[2] == 2){//四带两对
                        $rtnArr['type']     = 9;
                        $rtnArr['type2']    = 3;
                        $rtnArr['pklist']   = $arr;
                        $rtnArr['max']      = $newPkArr[4];
...
//                    $isThOne = 0;
            ...
            if($isThOne == 1 && count($thrPkArr) == $otherNum){
    ...
    //比赛结算
    public function clatimes($dTabUlist,$dTimeinfo,$dTabuinfo,$uid)
    {
        //$times = 9;
        //$dTabUlist = D('ddztabulist')->where('times = '.$times)->select();
        if($dTimeinfo->is_cla == 1){
            return ;
        }
        $landUserid = 0;
        foreach($dTabUlist as $k=>$v){
            $ddzTpArr[] = $v->user_id;
            if($v->is_dz == 1){
                $landUserid = $v->user_id;
            }
        }

        //$dTimeinfo = D('ddztimes')->where('id = '.$times)->find();
        $multiple_num = $dTimeinfo->multiple_num;

        //$dTabuinfo = D('ddztabulist')->where('user_id ='.$this->uid.' and times ='.$times)->find();


        //$uid = $this->uid;
        $allGcoins = $multiple_num * $dTimeinfo->base_coins;
        $dDUmod = D('ddzulist');
        $allUlist = $dDUmod->where('user_id in ('.join(',',$ddzTpArr).')')->select();//随便写的这种架架，用起来不方便，还是YII和laravel好用
//        echo $dDUmod->get_sql();

//        echo $allGcoins;
        $nowTime = time();
        if($dTabuinfo->is_dz == 1){
            //地主赢
            $wincoins = 0;
            $newUpUlArr = [];
            foreach($allUlist as $k=>$v){
                if($v->user_id == $uid){
                    $newUpUlArr[$v->user_id] = [
                        'glodcoins' => $v->glodcoins,
                        'total_matches' => $v->total_matches,
                        'win_matches' => $v->win_matches,
                        'fail_matches' => $v->fail_matches,
                    ];
                }else{
                    $tmpGdCs = $v->glodcoins - $allGcoins;
                    if($tmpGdCs > 0){
                        $wincoins += $allGcoins;
                    }else{
                        $wincoins += $v->glodcoins;
                        $tmpGdCs = 0;
                    }
                    $newUpUlArr[$v->user_id] = [
                        'lastuptime' => $nowTime,
                        'glodcoins' => $tmpGdCs,
                        'total_matches' => $v->total_matches +1,
                        //'win_matches' => $v->win_matches +1,
                        'fail_matches' => $v->fail_matches +1,
                    ];

                    D('ddzulist')->where('user_id = '.$v->user_id)->save($newUpUlArr[$v->user_id]);//无事务,无日志，裸奔，who怕who

                    $gdcLog = [
                        'user_id' => $v->user_id,
                        'oldcoins' => $v->glodcoins,
                        'coins' => $allGcoins,
                        'newcoins' => $tmpGdCs,
                        'acttp' => 2,
                        'times' => $dTimeinfo->id,
                    ...
                    D('ddzglodcoins_log')->add($gdcLog);
                }
            }

            $UpMyArr = [
                'lastuptime' => $nowTime,
                'glodcoins' => $newUpUlArr[$dTabuinfo->user_id]['glodcoins'] + $wincoins,
                'total_matches' => $newUpUlArr[$dTabuinfo->user_id]['total_matches'] +1,
                'win_matches' => $newUpUlArr[$dTabuinfo->user_id]['win_matches'] +1,
                //'fail_matches' => $v->fail_matches +1,
            ];

            D('ddzulist')->where('user_id = '.$dTabuinfo->user_id)->save($UpMyArr);

            $gdcLog = [
                'user_id' => $dTabuinfo->user_id,
                'oldcoins' => $newUpUlArr[$dTabuinfo->user_id]['glodcoins'],
                'coins' => $wincoins,
                'newcoins' => $UpMyArr['glodcoins'],
                'acttp' => 1,
                'times' => $dTimeinfo->id,
                'addtime' => $nowTime
            ];
            ...
            //农民赢
            $landAllCoin = 0;//地主的总金币数
            foreach($allUlist as $k=>$v){
                if($v->user_id == $landUserid){
                    $landAllCoin = $v->glodcoins;
                }
            }

            $surplusCoins = $landAllCoin - $allGcoins;//地主剩余的金额
            $farmerCoins = 0;//农民的金币
            if($surplusCoins > 0){
                //地主的金币大于棋牌的数量
                $farmerCoins = $allGcoins;
            }else{
                if($landAllCoin%2 == 0){
                    $surplusCoins = 0;
                    $farmerCoins = $landAllCoin;
                }else{
                    $surplusCoins = 1;
                    $farmerCoins = $landAllCoin -1;
                }
            }

            //开始更新农民的金币
            $farmerCoins = $farmerCoins/2;
            foreach($allUlist as $k=>$v){
                ...
                if($v->user_id == $landUserid){
                    ...
                    $newUpUlArr[$v->user_id] = [
                        'lastuptime' => $nowTime,
                        'glodcoins' => $v->glodcoins + $farmerCoins,
                        'total_matches' => $v->total_matches +1,
                        'win_matches' => $v->win_matches +1,
                    ];
                    ...
                    $gdcLog = [
                        ...
                        'oldcoins' => $v->glodcoins,
                        'coins' => $farmerCoins,
                        'newcoins' => $newUpUlArr[$v->user_id]['glodcoins'],
                        'acttp' => 1,
                        'times' => $dTimeinfo->id,
                        'addtime' => $nowTime
                    ];
            ...
            $UpMyArr = [
                'lastuptime' => $nowTime,
                'glodcoins' => $surplusCoins,
                'total_matches' => $newUpUlArr[$landUserid]['total_matches'] +1,
                //'win_matches' => $newUpUlArr[$dTabuinfo->user_id]['win_matches'] +1,
                'fail_matches' => $newUpUlArr[$landUserid]['fail_matches'] +1,
            ];

            D('ddzulist')->where('user_id = '.$landUserid)->save($UpMyArr);

            $gdcLog = [
                'user_id' => $dTabuinfo->user_id,
                'oldcoins' => $newUpUlArr[$landUserid]['glodcoins'],
                'coins' => $allGcoins,
                'newcoins' => $UpMyArr['glodcoins'],
                'acttp' => 2,
                'times' => $dTimeinfo->id,
    ...
    //不洗牌打法
    public function byrecordcreatepk()
    {
        $timeRes = D('ddztimes')->where('stepstate = 4')->order('id desc')->limit(2)->select();
        if(!$timeRes){
            return [];
        }
        if(count($timeRes) < 2){
        ...
        $res = D('ddztabulist')->where('times = '.$timeRes[1]->id)->select();
        $arr = [];
        foreach($res as $k=>$v){
